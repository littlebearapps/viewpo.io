...
export const FEATURES_DATA: FeatureDetail[] = [
  {
    ...
    description: 'Load any website at any screen size — even sites that block iframes. View a 27-inch desktop layout on your iPhone, pan around like a viewfinder, and verify your responsive breakpoints in seconds.',
    group: 'core',
    bullets: [
      '6 quick-access presets + 17-device library: iPhone SE through 4K UHD, plus any custom resolution',
      'Bypasses X-Frame-Options — loads any site, not just iframe-friendly ones',
      'Real CSS viewport injection — media queries fire at the simulated width, not a CSS zoom',
      'Dark/light mode override: cycle prefers-color-scheme without changing device settings',
      'iPad developers: see true desktop layouts — Safari\'s "Request Desktop Site" doesn\'t change the CSS viewport, but Viewpo does',
  ...
  },
  {
    ...
    description: 'Cloudflare, GitHub, Vercel, and Netlify in a single live feed. New deployments appear automatically — no refreshing. Pin your active branches to the top; toggle watch to get notified. Everything else stays quiet.',
    group: 'core',
    bullets: [
      ...
      'Each deployment shows commit message, branch, timestamp, and build status at a glance',
  ...
  },
  {
    ...
    description: 'Know the moment your preview is ready — without polling a dashboard or waiting for a Slack bot. Tap the notification, the viewer opens. Done.',
    group: 'core',
    bullets: [
      ...
      'Watch only the branches you care about — everything else stays quiet',
  ...
  },
  {
    ...
    description: 'Share a preview link that always shows the latest deployment for a branch — no re-sharing after each push. Or share a frozen link for a specific build. Recipients don\'t need a Viewpo account.',
    group: 'collaboration',
    bullets: [
  ...
  },
  {
    ...
    description: 'Clients approve or request changes from a share link. Teammates leave comments pinned directly on the live preview. No video call, no back-and-forth — just a clear approval trail.',
    group: 'collaboration',
    badge: 'Roadmap',
    bullets: [
      'Clients click Approve or Request Changes on the share link — no Viewpo account required',
      'Three comment types: pinned to a page element, screenshot annotation, or general thread',
      'Approval status visible per deployment: pending, approved, or changes requested',
  ...
  },
  {
    ...
    badge: 'Roadmap',
    bullets: [
      'Roles match how teams work: Owner controls everything, Members pin and share, Viewers see but can\'t change',
...
export const BETA_FAQ_ITEMS: Array<{ question: string; answer: string }> = [
  {
    ...
    answer: 'TestFlight is Apple\'s official beta testing platform — it\'s a free app you install from the App Store, just like any other app. You don\'t need an Apple Developer account or any technical setup — just a regular Apple ID. Once you\'re accepted into the Viewpo beta, we\'ll send you a TestFlight invite link. Tap it, install Viewpo, and you\'re in.',
  },
  {
